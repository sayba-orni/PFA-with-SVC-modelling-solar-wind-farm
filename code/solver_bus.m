function res = solver_bus(Y_base, Sbase, svc_bus, Vref_svc, SVC_Q_MAX, Pg, Qg, Pd, Qd)
...
%
% Inputs
%   Y_base     : 5x5 Ybus of the base network (complex)
%   Sbase      : system base MVA (scalar)
%   svc_bus    : bus index where SVC is connected (2..5)
%   Vref_svc   : |V| setpoint at SVC bus (pu)
%   SVC_Q_MAX  : |Qsvc| limit in MVAr
%   Pg, Qg     : 1x5 scheduled gen (pu). Slack P,Q solved from power flow.
%   Pd, Qd     : 1x5 loads (pu)
%
% Output
%   res : struct with fields
%         .converged .iter .err .mode
%         .svc_bus .B_svc .Qsvc_MVAr .svcType
%         .V_abs .V_deg
%         .genMW .genMVAr .loadMW .loadMVAr
%         .lines .I_mag .I_ang .Lmw .Lmvar .totPloss .totQloss

    N = 5;

    % convert MVAr limit to susceptance bounds (pu)
    Bmax =  (SVC_Q_MAX) / (max(Vref_svc,1e-6)^2 * Sbase);
    Bmin = -Bmax;

    % schedules (net injections)
    Psch = Pg - Pd;    % pu
    Qsch = Qg - Qd;    % pu
    ...
    while err>tol && iter<max_iter
        ...
        Yit = Y_base;  Yit(svc_bus,svc_bus) = Yit(svc_bus,svc_bus) + 1i*Bsvc;
        ...
        if mode=="REG"
            ...
            J2(:,idx) = 0;
            J4(:,idx) = 0;  J4(idx,idx) = -Vmag(svc_bus)^2;   % dQ/dB at svc bus
        end
        J  = [J1 J2; J3 J4];
        ...
        while alpha > 1/1024
            ...
            Ytr = Y_base; Ytr(svc_bus,svc_bus) = Ytr(svc_bus,svc_bus) + 1i*B_try;
        ...
        if ~accepted
            ...
            if mode=="REG"
                Vmag(2:N) = Vmag(2:N) + 1e-3*dVm; Vmag(svc_bus)=Vref_svc;
    ...
    % final quantities
    Yf = Y_base; Yf(svc_bus,svc_bus)=Yf(svc_bus,svc_bus)+1i*Bsvc;
    Vdeg = Vang*180/pi;
    ...
    svcType   = 'Inductive'; if Qsvc_MVAr>=0, svcType='Capacitive'; end

    % 7 original lines (currents & losses)
    lines = [1 2;1 3;2 3;2 4;2 5;3 4;4 5];
    Zlist = [0.02+0.06i; 0.08+0.24i; 0.06+0.25i; 0.06+0.18i; 0.04+0.12i; 0.01+0.03i; 0.08+0.24i];
    L = size(lines,1);
    I_mag=zeros(L,1); I_ang=zeros(L,1); Pl=zeros(L,1); Ql=zeros(L,1);
    for ee=1:L
        a = lines(ee,1); b = lines(ee,2);
        [I_mag(ee), I_ang(ee), Pl(ee), Ql(ee)] = current_and_lineloss(Vmag(a),Vdeg(a),Vmag(b),Vdeg(b),Zlist(ee),Sbase);
    end
